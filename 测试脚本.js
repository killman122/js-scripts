...
const jsname = "教程";//这里定义脚本的名称
const $ = Env(jsname);
let notify = $.isNode() ? require('./sendNotify') : '';      // 这里是 node（青龙属于node环境）通知相关的
const Notify = 1; //0为关闭通知，1为打开通知,默认为1
const debug = 0; //0为关闭调试，1为打开调试,默认为0
//////////////////////
let jiaocheng_data ="mtTV5o4akQanEnmiQTy/Rw==&285101@111"   //mtTV5o4akQanEnmiQTy/Rw==&285101                 //  process.env.jiaocheng_data               // 这里是 从青龙的 配置文件 读取你写的变量
let jiaocheng_dataArr = []; //数组用来存储多个cookie
...
!(async () => {//这个加叹号的异步函数相当于是主函数的入口,但是我习惯于写在程序的末尾
    if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
        ...
        for (let index = 0; index < jiaocheng_dataArr.length; index++) {
            ...
            console.log(index,jiaocheng_dataArr[0])
            console.log(typeof (jiaocheng_dataArr[0]),typeof (jiaocheng_dataArr))
            console.log(jiaocheng_dataArr[0].split('&'))
           data = jiaocheng_dataArr[index].split('&');      // 这里是分割你每个账号的每个小项,是整个cookie中的某个必要参数而不是整个的cookie
            ...
            console.log('开始 yy');
            // await yyyy();
            await $.wait(2 * 1000);//这里是设置等待的时间等待是两秒


            // 这里是开始做任务
            console.log('开始 zz');
            // await zzzzz();
            await $.wait(2 * 1000);//这里是设置等待的时间 等待的时间设置为两秒



            await SendMsg(msg);    // 与发送通知有关系调用sendNotify脚本发送通知
        }
    }

})()
        // .catch((e) => console.logErr(e))
        // .finally(() => $.done())//这里用来捕捉异常和完成整个函数
...
function signin() {
    return new Promise((resolve) => {
        ...
        if (debug) {
            console.log(`\n【debug】=============== 这是 签到 请求 url ===============`);
            // console.log(`这是测试加或者不加stringify的区别${url}`,url)
